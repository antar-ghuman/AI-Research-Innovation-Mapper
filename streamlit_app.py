...
"""
MINIMAL DEBUG VERSION - Replace your streamlit_app.py with this temporarily
This will help us find exactly what's causing the crash
"""

print("🚀 DEBUG: Starting minimal app")
...
st.title("🔬 DEBUG MODE - AI Research Innovation Mapper")
st.success("✅ Basic Streamlit is working!")

print("✅ DEBUG: Basic Streamlit loaded successfully")
...
def test_import(module_name, import_statement):
    try:
        print(f"🧪 DEBUG: Testing import: {module_name}")
...
test_import("networkx", "import networkx as nx")

# Test path setup
st.subheader("Testing Project Structure")
current_dir = os.path.dirname(os.path.abspath(__file__))
src_dir = os.path.join(current_dir, 'src')

if os.path.exists(src_dir):
    st.success(f"✅ src directory exists: {src_dir}")
    sys.path.insert(0, src_dir)
    st.info(f"Added to Python path: {src_dir}")
    
    if os.path.exists(os.path.join(src_dir, 'utils')):
        st.success("✅ utils directory exists")
        
        # Test utils imports
        test_import("utils.config", "from utils.config import get_config")
        test_import("utils.logging_config", "from utils.logging_config import init_logging")
        test_import("utils.demo_data", "from utils.demo_data import get_demo_data")
    else:
        st.error("❌ utils directory missing")
    
    if os.path.exists(os.path.join(src_dir, 'agents')):
        st.success("✅ agents directory exists")
        st.info(f"Agent files: {os.listdir(os.path.join(src_dir, 'agents'))}")
        
        # Test agent imports one by one
        st.subheader("Testing Agent Imports (Most Likely Culprit)")
        
        test_import("paper_discovery_agent", "from agents.paper_discovery_agent import GroqLLMWrapper")
        test_import("cross_domain_agent", "from agents.cross_domain_agent import CrossDomainAgent")
        test_import("orchestrator", "from agents.orchestrator import ResearchOrchestrator")
        
    else:
        st.error("❌ agents directory missing")
else:
    st.error(f"❌ src directory missing: {src_dir}")

# Test environment variables
st.subheader("Testing Environment")
groq_key = os.getenv("GROQ_API_KEY")
if groq_key:
    st.success(f"✅ GROQ_API_KEY is set (length: {len(groq_key)})")
else:
    st.warning("⚠️ GROQ_API_KEY is not set")
...
st.info("🎯 The first FAILED import is likely causing your deployment crash!")

print("✅ DEBUG: Minimal app completed successfully")